...
async function validateSession(env, request) {
  ...
  return { id: session.uid, email: session.email, plan: session.plan || 'free', is_superadmin: !!session.is_superadmin };
}
...
async function handleAdmin(request, env, corsHeaders, path, method) {
  ...
  if (path === '/api/v1/dashboard/admin/toggle-signups' && method === 'POST') {
    return adminToggleSignups(request, env, corsHeaders);
  }
...
async function adminListUsers(env, corsHeaders) {
...
async function adminListProjects(env, corsHeaders) {
...
async function adminListOrgs(env, corsHeaders) {
  const orgs = await env.DB.prepare(`
...
async function adminDeleteUser(env, userId, corsHeaders) {
